...
void Bet()
{
    ...
    //更新用户购买的彩票数、号码数
    ReCountUserALlTickets(lotteryCurrentLogin);
}
...
int CheckBetNumberUniqueness(char nums[])
{
    ...
    {
        for (int i = 0; strcmp(ticketTemp->data.numStr[i], empty) != 0; i++)
        {
...
{
    //用时间戳+3位数随机字符指定id，最大限度降低id重复率
    char emptyContainer[128] = {'\0'};
    char randStr[10] = {'\0'};
    srand(time(NULL));
...
//重新统计某个用户购买的所有权彩票数
void ReCountUserALlTickets(LotteryAccountLinkedList *user)
{
    TicketDataLinkedList *ticketDataTemp = ticketDataHead;
    unsigned cnt = 0;
    unsigned cntNums = 0;
    ...
    {
        //彩票上标记的账号和当前用户名相同
        if (strcmp(user->data.account.name, ticketDataTemp->data.purchaser) == 0)
        {
            cnt++;
            //遍历此彩票上的号码数
            for (int i = 0; strcmp(ticketDataTemp->data.numStr[i], empty) != 0; i++)
            {
                cntNums++;
            }
        }
    ...
    user->data.tickets = cnt;
    user->data.ticketNums = cntNums;
}
//重新统计奖池数据
void ReCountPrizePoolData()
{
    LotteryAccountLinkedList *user = lotteryAccountHead;
    TicketDataLinkedList *ticketTemp = ticketDataHead;
    if (releaseDataCurrent == NULL && ticketTemp == NULL)
    {
        return;
    }
    unsigned cnt = 0;
    ...
    releaseDataCurrent->data.totalSold = cnt;
    releaseDataCurrent->data.totalPrize = cnt * releaseDataCurrent->data.price;
}

//统计用户在某一期购买的彩票数及号码数
void CountUserSingleIssueTickets(ReleaseDataLinkedlist *issue, char userName[], int recvResult[])
{
    TicketDataLinkedList *ticketDataTemp = ticketDataHead;
    if (ticketDataTemp == NULL)
    {
    ...
    {
        //彩票上标记的账号和当前用户名相同
        if (strcmp(userName, ticketDataTemp->data.purchaser) == 0)
        {
            recvResult[0]++;
            //遍历此彩票上的号码数
            for (int i = 0; strcmp(ticketDataTemp->data.numStr[i], empty) != 0; i++)
